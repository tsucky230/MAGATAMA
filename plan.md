...
| ファイル | 内容 |
|---|---|
| `packages/yata-core/src/yata_core/domain/entities/base.py` | `Entity`（pydantic, frozen）, `EntityType` |
| `packages/yata-core/src/yata_core/domain/entities/relationships.py` | `Relationship`, `RelationshipType` |
| `packages/yata-core/src/yata_core/domain/value_objects/ids.py` | `EntityId(value: str)` |
| `packages/yata-core/src/yata_core/domain/value_objects/location.py` | `Location(file, line>=1, column>=0)` |
| `packages/yata-core/src/yata_core/infrastructure/storage/networkx_graph.py` | `NetworkXKnowledgeGraph`（`entities.add/delete/all`, `relationships.add/delete/all` を持つ） |
| `packages/yata-mcp/src/yata_mcp/server/protocol.py` | FastMCP サーバー本体。`@mcp.tool()` でツール登録。**今回ツールを追加するのはこのファイル** |

注意: `packages/yata-mcp/src/yata_mcp/server/mcp_server.py`（`YataMcpServer`）は CLI
（`yata query` 等）から使われる別実装。今回の必須スコープは `protocol.py` のみ（§4 Task 4 参照）。
...
**新規ファイル**: `packages/yata-core/src/yata_core/infrastructure/storage/comp_index_reader.py`
...
- ⚠️ 上記スケルトンの `file_path` 解決はリスト線形探索になっている。実装時は
  ...
  O(1) で引くこと
- `__init__.py`（`packages/yata-core/src/yata_core/infrastructure/storage/__init__.py`）に
...
**新規ファイル**: `packages/yata-core/src/yata_core/application/usecases/comp_usecase.py`
...
- `relationships.delete()` のシグネチャは
  `packages/yata-core/src/yata_core/infrastructure/storage/in_memory_repository.py:103` を
  実装前に必ず確認し、引数の形（個別引数か Relationship オブジェクトか）を合わせること
- `usecases/__init__.py` に再エクスポートを追加すること

### Task 3: MCP ツールの追加（`protocol.py`）

**変更ファイル**: `packages/yata-mcp/src/yata_mcp/server/protocol.py`
...
**変更ファイル**: `packages/yata-mcp/src/yata_mcp/cli/main.py`

動作確認を CLI から行えるよう、`yata comp-load <path>` コマンドを追加する。
ただし `cli/main.py` は `YataMcpServer`（`mcp_server.py`）ベースなので、
`YataMcpServer.__init__` にも `LoadCompIndexUseCase` を組み込み、
...
**新規ファイル**: `packages/yata-core/tests/infrastructure/test_comp_index_reader.py`
...
**新規ファイル**: `packages/yata-core/tests/application/test_comp_usecase.py`（UseCase）:
...
**変更ファイル**: `packages/yata-mcp/tests/test_protocol.py`（MCPツール）:
...
- ツール総数をアサートしているテストがあれば 34 → 36 に更新する（実装前に
  `grep -rn "34" packages/yata-mcp/tests/` で確認すること）
...
uv run pytest packages/yata-core/tests/infrastructure/test_comp_index_reader.py -v
uv run pytest packages/yata-core/tests/application/test_comp_usecase.py -v
uv run pytest packages/yata-mcp/tests/test_protocol.py -v
...
1. comP でインデックス済みのワークスペースを用意する（例: `e:/dev/comP` 自体に
   ...
   プロジェクトをインデックスする）
2. YATA MCP サーバーを起動: `uv run yata serve`
...
- [ ] `read_external_graph` / `get_external_graph_info` が `yata serve` のツール一覧に出る
