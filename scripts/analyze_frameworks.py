...
FRAMEWORKS = [
    ...
    FrameworkConfig(
        "React", "react", "ts", ["packages/**/*.ts", "packages/**/*.tsx", "packages/**/*.js"]
    ),
    ...
    try:
        ...
        for f in all_files:
            ...
            for exclude in fw.exclude_patterns:
                if f.match(exclude) or any(
                    p in str(f) for p in ["__pycache__", "node_modules", "test", "spec", ".git"]
                ):
                    excluded = True
                    break
...
def main():
    """Main entry point."""
    base_path = Path("/home/nahisaho/GitHub/MAGATAMA/frameworks")
    parsers = get_parsers()
    ...
    for r in sorted(results, key=lambda x: x.get("entities", 0), reverse=True):
        if "error" not in r:
            print(
                f"{r['name']:<20} {r['language']:<8} {r['files_processed']:<8} {r['entities']:<10} {r['relationships']:<10}"
            )
        else:
