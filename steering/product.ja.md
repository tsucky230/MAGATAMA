...
> 兄弟ツール [comP](https://github.com/tsucky230/comP)（VSCode 拡張のコード
> インデクサー）との連携（comP Bridge）を追加したものです。以下のビジョンは
> フォーク元 YATA から継承しています。
...
> MAGATAMA（勾玉）は、広大な情報から本質だけを抽出し高密度に凝縮する勾玉のように、コードベースを知識グラフへ圧縮して AI に「真のコンテキスト」を最小トークンで提供し、幻覚（ハルシネーション）のない正確なコード生成を支援します。comP が作るインデックス（地図）と組み合わせることで効果が倍増します。
...
MAGATAMAは、AI Codingアシスタント（GitHub Copilot、Claude、Cursor等）に対して、プログラミング言語やフレームワークの最新ドキュメントとコード構造を提供するMCP（Model Context Protocol）サーバーです。兄弟ツール comP（VSCode 拡張のコードインデクサー）が作るインデックスを取り込めるのが特長です。

> 従来のAIコーディングツールは、トレーニングデータに含まれる古い情報に依存しているため、最新のAPIや推奨されるベストプラクティスを認識できません。MAGATAMAは、ソースコードの静的解析による知識グラフと、comP インデックスの取り込みを組み合わせることで、AIが全ソースを読まずに正確な文脈を得られるようにし、この課題を解決します。
...
> Context7などの競合製品はクラウド依存でプライバシー懸念がある。MAGATAMAは完全ローカル実行で差別化。さらに comP（VSCode 拡張）と組み合わせると、既存インデックスを再パースなしで取り込めるため導入コストが低い。
...
| ---------- | --------- | ---------- | ------------------- |
| Context7 | クラウドDB充実 | プライバシー懸念、5言語のみ | 完全ローカル、24言語対応 |
| CodeGraph MCP | 高速解析 | フレームワーク知識なし | 47フレームワーク知識グラフ |
| comP 単体 | 高速ローカルインデックス | 知識グラフ分析が限定的 | comP の地図を 36 MCP ツールで横断分析 |
...
1. **コード解析エンジン**
   - **Description**: 24言語対応のTree-sitter AST解析
...
3. **MCP Server**
   - **Description**: 36種類のMCPツール提供（YATA 由来 34 + comP Bridge 2）
   - **User Value**: Claude/Copilot/Cursorとの統合
   - **Priority**: P0 (Critical) ✅ 実装済み

> **comP Bridge（MAGATAMA 独自・YATA にない追加機能）** ✅ 実装済み P0
> comP の `.comp/index.db` を再パースなしで知識グラフに取り込む
> （`read_external_graph` / `get_external_graph_info`）。VSCode の comP で作った
> 地図をそのまま AI 分析に活用でき、トークンを大幅削減。
