...
mode and converts it into MAGATAMA domain entities/relationships.
"""
...
def resolve_db_path(path: str | Path) -> Path:
    ...
    raise CompIndexNotFoundError(
        f"comP index not found at {path!r} (tried: " + ", ".join(str(c) for c in candidates) + ")"
    )
...
class CompIndexReader:
    ...
    def _read_all(self, conn: sqlite3.Connection, alias: str, db_path: Path) -> CompIndexData:
        ...
        for row in conn.execute("SELECT id, path, language FROM files ORDER BY id"):
            fid = int(row["id"])
        ...
        for row in conn.execute(
            ...
            data.entities.append(
                Entity(
                    id=eid,
                    name=row["name"],
                    type=_KIND_TO_ENTITY_TYPE.get(row["kind"], _DEFAULT_ENTITY_TYPE),
                    location=Location(
                        file=file_path,
        ...
        for row in conn.execute("SELECT from_id, to_id, kind FROM edges"):
            ...
            data.relationships.append(
                Relationship(
                    source_id=src,
                    target_id=dst,
                    type=_EDGE_TO_RELATIONSHIP_TYPE.get(row["kind"], _DEFAULT_RELATIONSHIP_TYPE),
