...
This module provides TypeScript/TSX parsing capabilities for
extracting entities from TypeScript source code.
"""
...
class TypeScriptParser:
    """Parser for TypeScript/TSX source code using tree-sitter.
    ...
    def parse_string(self, code: str, file_path: str = "<string>") -> ParseResult:
        """Parse TypeScript code from a string.
        ...
        Returns:
            ParseResult with extracted entities
        """
        # Use TSX parser for .tsx files, TypeScript for others
    ...
    def _extract_from_node(
        ...
        # Interface declarations
            ...
            entity = self._extract_interface(node, file_path)
            if entity:
                entities.append(entity)
                if parent_id:
                    relationships.append(
        ...
        # Type alias declarations
            ...
            entity = self._extract_type_alias(node, file_path)
            if entity:
                entities.append(entity)
                if parent_id:
                    relationships.append(
        ...
        # Enum declarations
            ...
            entity = self._extract_enum(node, file_path)
            if entity:
                entities.append(entity)
                if parent_id:
                    relationships.append(
    ...
    def _extract_function(self, node: Node, file_path: str) -> FunctionEntity | None:
        ...
        if params_node:
            for param in params_node.children:
                if param.type in ("required_parameter", "optional_parameter"):
                    ...
                    if param_name and param_name.text:
                        param_name_str = param_name.text.decode("utf-8")
                        param_type_str: str | None = None
                        if param_type_node and param_type_node.text:
                            param_type_str = param_type_node.text.decode("utf-8")
                        parameters.append((param_name_str, param_type_str))
        ...
        if return_type_node:
            # Skip the colon
            type_node = return_type_node.child(0) if return_type_node.child_count > 0 else None
    ...
    def _extract_method(self, node: Node, file_path: str, class_id: str) -> MethodEntity | None:
        ...
        is_static = any(
            child.type == "static"
            or (child.type == "accessibility_modifier" and child.text and b"static" in child.text)
            for child in node.children
        )
    ...
    def _extract_interface(self, node: Node, file_path: str) -> InterfaceEntity | None:
        """Extract an interface entity."""
        name_node = node.child_by_field_name("name")
        if not name_node:
            return None
        ...
        for child in node.children:
            if child.type == "extends_type_clause":
                for type_child in child.children:
                    if type_child.type == "type_identifier" and type_child.text:
                        extends.append(type_child.text.decode("utf-8"))

        return InterfaceEntity(
            id=EntityId(value=self._generate_id("interface")),
            name=name,
    ...
    def _extract_type_alias(self, node: Node, file_path: str) -> TypeEntity | None:
        ...
        if value_node and value_node.text:
            type_value = value_node.text.decode("utf-8")

        return TypeEntity(
            id=EntityId(value=self._generate_id("type")),
            name=name,
            ...
            type_definition=type_value,
        )

    def _extract_enum(self, node: Node, file_path: str) -> EnumEntity | None:
        ...
        if body:
            for child in body.children:
                if child.type == "enum_assignment":
                    ...
                    if member_name and member_name.text:
                        members.append(member_name.text.decode("utf-8"))
                elif child.type == "property_identifier" and child.text:
                    members.append(child.text.decode("utf-8"))

        return EnumEntity(
            id=EntityId(value=self._generate_id("enum")),
            name=name,
    ...
    def _extract_calls_relationships(
        ...
        self._find_calls_in_node(
            root, code, relationships, function_name_to_id, current_function_id=None
        )
    ...
    def _find_calls_in_node(
        ...
        if node.type in ("function_declaration", "method_definition", "arrow_function"):
            ...
            if body and func_id:
                for child in body.children:
                    self._find_calls_in_node(
                        child, code, relationships, function_name_to_id, func_id
                    )
            elif body:
                # Still traverse body even without func_id
                for child in body.children:
                    self._find_calls_in_node(
                        child, code, relationships, function_name_to_id, current_function_id
                    )
                        ...
                        if import_child.type == "import_clause":
                            for clause_child in import_child.children:
                                if clause_child.type == "identifier" and clause_child.text:
                                    ...
                                    if name_node and name_node.text:
                                        imported_names.append(
                                            f"* as {name_node.text.decode('utf-8')}"
                                        )
