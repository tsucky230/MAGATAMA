...
class DartParser:
    ...
    def __init__(self) -> None:
        ...
        self._mixin_pattern = re.compile(
            r"mixin\s+(\w+)(?:<[^>]+>)?(?:\s+on\s+([^{]+))?\s*\{", re.MULTILINE
        )
        self._extension_pattern = re.compile(
            r"extension\s+(\w+)?\s+on\s+(\w+(?:<[^>]+>)?)\s*\{", re.MULTILINE
        )
        ...
        self._enum_pattern = re.compile(r"enum\s+(\w+)\s*\{", re.MULTILINE)
        self._typedef_pattern = re.compile(r"typedef\s+(\w+)(?:<[^>]+>)?\s*=", re.MULTILINE)
        ...
        for match in self._class_pattern.finditer(code):
            ...
            if class_end > class_start:
                ...
                self._extract_methods(
                    class_body, file_path, line_num, class_id, entities, relationships
                )
