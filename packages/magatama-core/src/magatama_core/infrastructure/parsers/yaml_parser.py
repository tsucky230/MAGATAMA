...
class YAMLParser:
    ...
    def parse_string(self, code: str, file_path: str = "<string>") -> ParseResult:
        ...
        self._extract_from_node(
            tree.root_node, code, file_path, entities, relationships, module_id, ""
        )
    ...
    def _extract_from_node(
        ...
        if node.type == "block_mapping_pair":
            self._extract_mapping_pair(
                node, code, file_path, entities, relationships, parent_id, path_prefix
            )
        elif node.type == "block_sequence_item":
            self._extract_sequence_item(
                node, code, file_path, entities, relationships, parent_id, path_prefix
            )
        else:
            for child in node.children:
                self._extract_from_node(
                    child, code, file_path, entities, relationships, parent_id, path_prefix
                )
    ...
    def _extract_mapping_pair(
        ...
        if key_node:
            ...
            key_entity = ModuleEntity(
                ...
                type=EntityType.MODULE,
                location=Location(
                    file=file_path, line=key_node.start_point[0] + 1, column=key_node.start_point[1]
                ),
                file_path=file_path,
            )
            ...
            if value_node:
                for child in value_node.children:
                    self._extract_from_node(
                        child, code, file_path, entities, relationships, key_id, current_path
                    )
    ...
    def _extract_sequence_item(
        ...
        for child in node.children:
            if child.type not in ["-"]:
                self._extract_from_node(
                    child, code, file_path, entities, relationships, item_id, current_path
                )
