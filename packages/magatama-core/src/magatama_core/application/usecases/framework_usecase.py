...
class FrameworkKnowledgeUseCase:
                ...
                if isinstance(frameworks_data, list):
                    ...
                    for fw_data in frameworks_data:
                        ...
                        self._framework_info[fw_name] = FrameworkInfo(
                            ...
                            graph_path=Path(fw_data.get("graph_file", ""))
                            if fw_data.get("graph_file")
                            else None,
                        )
    ...
    def search_all_frameworks(
        ...
        for fw_name in self._framework_info:
            result = self.search_framework(fw_name, query, entity_type, limit_per_framework)
            if result.entities:
...
class CodeContextUseCase:
    ...
    def generate_context(
        ...
        try:
            ...
            contained_in = [
                r for r in all_rels if r.target_id == eid and r.type.value == "contains"
            ]
            ...
            context: dict[str, Any] = {
                ...
                "calls": [self._get_entity_brief(r.target_id) for r in calls[:max_related]],
                "called_by": [self._get_entity_brief(r.source_id) for r in called_by[:max_related]],
                "imports": [self._get_entity_brief(r.target_id) for r in imports[:max_related]],
                "contains": [self._get_entity_brief(r.target_id) for r in contains[:max_related]],
                "contained_in": [
                    self._get_entity_brief(r.source_id) for r in contained_in[:max_related]
                ],
            }
...
class FrameworkSemanticSearchUseCase:
    ...
    def search(
        ...
        for fw_name, fw_info in self._frameworks.items():
            ...
            try:
                with open(fw_info.file_path) as f:
                    data = json.load(f)
                ...
                for entity_data in data.get("entities", []):
                    ...
                    score = self._calculate_relevance(
                        entity_name, entity_type, docstring, file_path, query_lower, query_words
                    )
...
class CodeRecommendationUseCase:
    ...
    def _calculate_relevance(
        ...
        # Apply weights
        return name_score * 0.4 + docstring_score * 0.3 + context_score * 0.2 + usage_score * 0.1
    ...
    def _calculate_entity_relevance(
        ...
        return name_score * 0.4 + docstring_score * 0.3 + context_score * 0.2
...
class HybridSearchUseCase:
    ...
    def hybrid_search(
        ...
        if self._frameworks_dir:
            fw_results = self._search_frameworks(query_lower, query_words, detected_frameworks)
    ...
    def _group_similar_results(
        ...
        for result in results:
            ...
            if any(
                name_lower == seen or name_lower in seen or seen in name_lower
...
class CodeQualityUseCase:
    ...
    def analyze_quality(
        ...
        overall_status = self._determine_status(overall_score, 70, 50, lower_is_better=False)
    ...
    def track_evolution(
        ...
        if not self._git_available:
            ...
            return EvolutionResult(
                ...
                is_hotspot=False,
                timeline_data=[
                    {"error": "GitPython not available. Install with: pip install gitpython"}
                ],
            )
        ...
        for entity in similar[:5]:
            ...
            for rel in all_rels:
                if rel.source_id == entity.id and rel.type.value == "imports":
                    ...
                    if target:
                        imports.add(
                            f"from {target.location.file.replace('/', '.').replace('.py', '')} import {target.name}"
                        )
...
class PatternDetectionUseCase:
    ...
    def _get_pattern_suggestions(self, pattern_name: str, entity: Entity) -> list[str]:
        ...
        if pattern_name == "Singleton":
            suggestions.append(
                "Consider using dependency injection instead of Singleton for testability"
            )
...
class APICompatibilityUseCase:
    ...
    KNOWN_ISSUES: dict[str, dict[str, list[dict[str, Any]]]] = {
        "django": {
            ...
            "4.2": [
                {
                    "api": "default_app_config",
                    "type": "deprecated",
                    "hint": "Remove default_app_config from __init__.py",
                },
            ],
        },
        "fastapi": {
            "0.100": [
                {
                    "api": "on_event",
                    "type": "deprecated",
                    "hint": "Use lifespan context manager instead",
                },
            ],
        },
        "react": {
            "18.0": [
                {
                    ...
                    "type": "removed",
                    "hint": "Use useEffect hook instead",
                },
                {
                    "api": "componentWillReceiveProps",
                    "type": "removed",
                    "hint": "Use getDerivedStateFromProps",
                },
            ],
        },
        "flask": {
            "2.0": [
                {
                    ...
                    "type": "deprecated",
                    "hint": "Use app context or flask-caching",
                },
            ],
        },
    }
