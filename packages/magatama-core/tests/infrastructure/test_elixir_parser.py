...
from magatama_core.infrastructure.parsers import ElixirParser
...
def test_elixir_rich_constructs():
    """Parse an Elixir file exercising module/def/defp/defmacro/defprotocol extraction."""
    from magatama_core.infrastructure.parsers import ElixirParser

    code = """
defmodule Calculator do
  @moduledoc "A calculator."
...
"""
    result = ElixirParser().parse_string(code, "sample.ex")
    assert len(result.entities) >= 1
    assert len(result.errors) == 0
