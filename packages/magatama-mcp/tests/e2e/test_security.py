...
Verifies that MAGATAMA operates fully locally without network access.
...
""")
            ...
            # Search
            search = await server.call_tool("search_entities", {"query": "App"})
            assert search["total_count"] > 0

            # Save
            graph_file = Path(tmpdir) / "graph.json"
            save_result = await server.call_tool("save_graph", {"file_path": str(graph_file)})
            assert save_result["success"]
            assert graph_file.exists()

            # Load
            server2 = MagatamaMcpServer()
            load_result = await server2.call_tool("load_graph", {"file_path": str(graph_file)})
...
class TestNoTelemetry:
    ...
    def test_no_environment_telemetry_vars(self):
        ...
        telemetry_vars = [
            ...
            "MAGATAMA_ANALYTICS",
            "MAGATAMA_TRACKING",
            "MAGATAMA_REPORT_USAGE",
            "MAGATAMA_SEND_METRICS",
        ]
    ...
    def test_no_external_service_calls(self):
        ...
        for module in [magatama_core, magatama_mcp]:
...
class TestOfflineOperation:
    ...
    def test_no_import_of_network_libraries(self):
        ...
        for lib in network_libs:
            ...
            assert lib not in sys.modules or lib not in str(magatama_core.__file__)
...
class TestDataPrivacy:
    ...
    async def test_no_data_in_temp_outside_project(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            server = MagatamaMcpServer()
            await server.call_tool("parse_directory", {"directory": str(project)})
...
class TestInputValidation:
    ...
    async def test_path_traversal_prevention(self):
        ...
        for path in malicious_paths:
            result = await server.call_tool("parse_file", {"file_path": path})
    ...
    async def test_special_characters_in_path(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            server = MagatamaMcpServer()
            result = await server.call_tool("parse_file", {"file_path": str(special_file)})
...
''',
            ...
            server = MagatamaMcpServer()
            result = await server.call_tool("parse_file", {"file_path": str(unicode_file)})
            ...
            assert (
                "errors" in result or result.get("success") or result.get("entities_count", 0) >= 0
            )
...
class TestSecurityAudit:
    ...
    def test_sql_injection_prevention(self):
        ...
        from magatama_core.infrastructure.storage.sqlite_storage import SQLiteKnowledgeGraph
