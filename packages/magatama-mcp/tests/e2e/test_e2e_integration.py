...
from magatama_core.infrastructure.storage import SQLiteKnowledgeGraph
...
""")
    ...
    async def test_python_project_full_indexing(self, sample_python_project: Path):
        ...
        result = await server.call_tool(
            "parse_directory", {"directory": str(sample_python_project)}
        )
    ...
    async def test_typescript_project_full_indexing(self, sample_typescript_project: Path):
        ...
        result = await server.call_tool(
            "parse_directory",
            {"directory": str(sample_typescript_project), "patterns": ["**/*.ts"]},
        )
    ...
    async def test_entity_search_workflow(self, sample_python_project: Path):
        ...
        server = MagatamaMcpServer()
        await server.call_tool("parse_directory", {"directory": str(sample_python_project)})

        # Search by name
        results = await server.call_tool("search_entities", {"query": "User"})
    ...
    async def test_entity_retrieval_workflow(self, sample_python_project: Path):
        ...
        # Get entity by ID
        result = await server.call_tool("get_entity", {"entity_id": user_class.id.value})
    ...
    async def test_relationship_navigation(self, sample_python_project: Path):
        ...
        related = await server.call_tool(
            "get_related_entities", {"entity_id": admin_class.id.value, "depth": 1}
        )
...
'''
        ...
        result2 = await server.call_tool(
            "incremental_parse", {"directory": str(sample_python_project)}
        )
    ...
    async def test_graph_persistence_workflow(self, sample_python_project: Path):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            server1 = MagatamaMcpServer()
            await server1.call_tool("parse_directory", {"directory": str(sample_python_project)})

            # Save graph
            save_result = await server1.call_tool("save_graph", {"file_path": str(graph_path)})
            assert save_result["success"]
            assert graph_path.exists()

            # Load into new server
            server2 = MagatamaMcpServer()
            load_result = await server2.call_tool("load_graph", {"file_path": str(graph_path)})
...
class TestE2EMcpProtocol:
    ...
    def test_mcp_resource_registered(self):
        ...
        assert "magatama://graph/stats" in resource_uris
    ...
    async def test_mcp_error_handling(self):
        ...
        with pytest.raises(ValueError, match="Unknown resource"):
            await server.read_resource("magatama://unknown")
...
""")
            ...
            # Parse Python file
            result_py = await server.call_tool(
                "parse_file", {"file_path": str(project / "backend.py")}
            )
            assert result_py["success"]

            # Parse TypeScript file
            result_ts = await server.call_tool(
                "parse_file", {"file_path": str(project / "frontend.ts")}
            )
...
''')
            ...
            start = time.time()
            result = await server.call_tool("parse_directory", {"directory": str(project)})
            elapsed = time.time() - start
...
""")
            ...
            server = MagatamaMcpServer()
            await server.call_tool("parse_directory", {"directory": str(project / "tests")})

            # Search for test classes
            results = await server.call_tool(
                "search_entities", {"query": "Test", "entity_type": "class"}
            )
...
''')
            ...
            server = MagatamaMcpServer()
            await server.call_tool("parse_file", {"file_path": str(project / "models.py")})

            # Find SuperAdmin class
            results = await server.call_tool(
                "search_entities", {"query": "SuperAdmin", "entity_type": "class"}
            )
            ...
            # Get related entities
            related = await server.call_tool(
                "get_related_entities", {"entity_id": super_admin["id"], "depth": 3}
            )
