...
class TestMagatamaMcpServer:
    """Tests for MagatamaMcpServer."""
    ...
    def test_server_initialization(self, server: MagatamaMcpServer) -> None:
        """Test server initializes with correct name."""
        assert server.name == "magatama-test"
        assert server._knowledge_graph is not None

    def test_server_has_tools(self, server: MagatamaMcpServer) -> None:
    ...
    def test_server_has_resources(self, server: MagatamaMcpServer) -> None:
    ...
    @pytest.mark.asyncio
    async def test_parse_file_tool(self, server: MagatamaMcpServer, tmp_path) -> None:
...
''')
        ...
        result = await server.call_tool("parse_file", {"file_path": str(test_file)})

        assert result["success"] is True
        assert result["entities_count"] >= 1

    @pytest.mark.asyncio
    async def test_parse_directory_tool(self, server: MagatamaMcpServer, tmp_path) -> None:
    ...
    @pytest.mark.asyncio
    async def test_search_entities_tool(self, server: MagatamaMcpServer, tmp_path) -> None:
...
""")
        ...
        # Search for entities
        result = await server.call_tool(
            "search_entities", {"query": "calculate", "entity_type": "function"}
        )
    ...
    @pytest.mark.asyncio
    async def test_get_entity_tool(self, server: MagatamaMcpServer, tmp_path) -> None:
        ...
        search_result = await server.call_tool("search_entities", {"query": "my_function"})

        if search_result["entities"]:
            entity_id = search_result["entities"][0]["id"]
            result = await server.call_tool("get_entity", {"entity_id": entity_id})
            assert result["entity"] is not None
            assert result["entity"]["name"] == "my_function"

    @pytest.mark.asyncio
    async def test_get_related_entities_tool(self, server: MagatamaMcpServer, tmp_path) -> None:
...
""")
        ...
        search_result = await server.call_tool(
            "search_entities", {"query": "Parent", "entity_type": "class"}
        )

        if search_result["entities"]:
            entity_id = search_result["entities"][0]["id"]
            result = await server.call_tool(
                "get_related_entities", {"entity_id": entity_id, "depth": 1}
            )
            # Should find related methods
            assert "related_entities" in result

    @pytest.mark.asyncio
    async def test_unknown_tool_raises_error(self, server: MagatamaMcpServer) -> None:
...
class TestMcpServerResources:
    ...
    @pytest.mark.asyncio
    async def test_read_stats_resource(self, server: MagatamaMcpServer, tmp_path) -> None:
...
""")
        ...
        # Read stats resource
        content = await server.read_resource("magatama://graph/stats")
...
class TestMcpServerPersistence:
    ...
    @pytest.mark.asyncio
    async def test_save_graph_tool(self, server: MagatamaMcpServer, tmp_path) -> None:
        ...
        result = await server.call_tool("save_graph", {"file_path": str(save_path)})
    ...
    @pytest.mark.asyncio
    async def test_load_graph_tool(self, server: MagatamaMcpServer, tmp_path) -> None:
        ...
        new_server = MagatamaMcpServer(name="magatama-new")
        result = await new_server.call_tool("load_graph", {"file_path": str(save_path)})

        assert result["success"] is True
        assert result["entities_count"] >= 1

        # Verify entity is searchable
        search_result = await new_server.call_tool("search_entities", {"query": "LoadTest"})
        assert len(search_result["entities"]) >= 1

    @pytest.mark.asyncio
    async def test_load_nonexistent_file(self, server: MagatamaMcpServer, tmp_path) -> None:
        """Test load_graph with non-existent file."""
        result = await server.call_tool(
            "load_graph", {"file_path": str(tmp_path / "nonexistent.json")}
        )
...
class TestIncrementalParse:
    ...
    def server(self) -> MagatamaMcpServer:
        """Create a server instance."""
        return MagatamaMcpServer(name="magatama-incremental-test")

    @pytest.mark.asyncio
    async def test_incremental_parse_initial(self, server: MagatamaMcpServer, tmp_path) -> None:
        """Test incremental_parse on first run."""
        (tmp_path / "module_a.py").write_text("def func_a(): pass")
        (tmp_path / "module_b.py").write_text("def func_b(): pass")

        result = await server.call_tool(
            "incremental_parse", {"directory": str(tmp_path), "patterns": ["*.py"]}
        )
    ...
    @pytest.mark.asyncio
    async def test_incremental_parse_skip_unchanged(
    ...
    @pytest.mark.asyncio
    async def test_incremental_parse_reparse_modified(
        self, server: MagatamaMcpServer, tmp_path
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_incremental_parse_removes_deleted_files(
        self, server: MagatamaMcpServer, tmp_path
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_incremental_parse_tool_registered(self, server: MagatamaMcpServer) -> None:
