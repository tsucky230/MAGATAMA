"""MAGATAMA CLI - Command Line Interface.
...
@click.group()
@click.version_option(version=__version__, prog_name="MAGATAMA")
@click.pass_context
def cli(ctx: click.Context) -> None:
    """MAGATAMA - AI Coding Support via Model Context Protocol.

    MAGATAMA (勾玉) provides knowledge graph context to AI coding assistants
    via the Model Context Protocol (MCP).
    """
    ctx.ensure_object(dict)
    ctx.obj["server"] = MagatamaMcpServer()
...
def parse(
    ...
    if path_obj.is_file():
        ...
        result = asyncio.run(server.call_tool("parse_file", {"file_path": path}))
...
def serve(transport: str, port: int) -> None:
    ...
    if transport == "sse":
        console.print("[blue]Starting MAGATAMA MCP Server (SSE)[/blue]")
...
@cli.command()
def info() -> None:
    """Show MAGATAMA server information."""
    import asyncio

    from magatama_mcp.server.protocol import create_mcp_server

    table = Table(title="MAGATAMA Server Information")
    table.add_column("Property", style="cyan")
    table.add_column("Value", style="green")

    table.add_row("Name", "MAGATAMA (勾玉)")
...
def save(ctx: click.Context, output: str) -> None:
    ...
    result = asyncio.run(server.call_tool("save_graph", {"file_path": str(output_path)}))

    if result["success"]:
        console.print(
            f"[green]✓[/green] Saved {result['entities_count']} entities, {result['relationships_count']} relationships"
        )
    else:
...
def load(ctx: click.Context, input_file: str) -> None:
    ...
    result = asyncio.run(server.call_tool("load_graph", {"file_path": str(input_path)}))

    if result["success"]:
        console.print(
            f"[green]✓[/green] Loaded {result['entities_count']} entities, {result['relationships_count']} relationships"
        )
    else:
        console.print(f"[red]✗[/red] Error: {result.get('error', 'Unknown error')}")
        raise SystemExit(1)
...
def query(
    ...
    Examples:
        ...
        magatama query "User" --max-results 5 --json

        magatama query "config" --graph ./my_graph.json
    """
    import asyncio
    ...
    if graph:
        result = asyncio.run(server.call_tool("load_graph", {"file_path": graph}))
    ...
    # Execute search
    result = asyncio.run(server.call_tool("search_entities", search_args))
    ...
    if output_json:
        ...
        for entity in entities:
            ...
            if entity.get("docstring"):
                doc = (
                    entity["docstring"][:80] + "..."
                    if len(entity.get("docstring", "")) > 80
                    else entity.get("docstring", "")
                )
                console.print(f"    📝 {doc}")
            console.print()
...
def stats(
    ...
    Examples:
        ...
        magatama stats --graph ./my_graph.json
    ...
    # Read stats resource
    stats_data = asyncio.run(asyncio.to_thread(server.read_resource_sync, "magatama://graph/stats"))
...
def validate(
    ...
    Examples:
        ...
        magatama validate --graph ./my_graph.json

        magatama validate --graph ./my_graph.json --repair

        magatama validate --json
    """
    import asyncio
    import json as json_module

    from magatama_core.application.services import GraphValidator
    ...
    if graph:
        ...
        result = asyncio.run(server.call_tool("load_graph", {"file_path": str(graph_path)}))
        if not result.get("success"):
            console.print(
                f"[red]✗[/red] Failed to load graph: {result.get('error', 'unknown error')}"
            )
            raise SystemExit(1)
    ...
    # Run validation
    console.print("[blue]Validating graph integrity...[/blue]")
    ...
    if output_json:
        ...
        if validation_result.is_valid:
            console.print("[green]✓[/green] Graph is valid")
        else:
            console.print("[red]✗[/red] Graph has integrity issues")
    ...
    if repair and not validation_result.is_valid:
        ...
        if repair_result.success:
            console.print("[green]✓[/green] Repair completed")
            ...
            if graph:
                ...
                save_result = asyncio.run(server.call_tool("save_graph", {"file_path": graph}))
                if save_result.get("success"):
                    console.print("[green]✓[/green] Graph saved")
                else:
                    console.print(f"[red]✗[/red] Failed to save: {save_result.get('error')}")
        else:
            console.print("[red]✗[/red] Repair failed")
...
def benchmark(
    ...
    Examples:
        ...
        magatama benchmark ./src --pattern "**/*.py" --json
    """
    import json as json_module

    from magatama_core.application.services import measure_indexing_performance
...
def watch(
    ...
    Examples:
        ...
        magatama watch ./src --pattern "**/*.py" --debounce 2.0
    ...
    def process_changes() -> None:
        ...
        if output:
            save_result = asyncio.run(server.call_tool("save_graph", {"file_path": output}))
