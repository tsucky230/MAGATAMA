...
import pytest
from yata_core.infrastructure.parsers import DartParser
...
"""
        result = parser.parse_string(code, "test.dart")
        classes = [e for e in result.entities if e.type == EntityType.CLASS and e.name == "MyWidget"]
        assert len(classes) == 1
...
"""
        result = parser.parse_string(code, "test.dart")
        classes = [e for e in result.entities if e.type == EntityType.CLASS and e.name == "MyService"]
        assert len(classes) == 1
