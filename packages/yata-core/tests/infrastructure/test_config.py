...
class TestStorageConfig:
    ...
    def test_storage_config_custom_values(self) -> None:
        ...
        config = StorageConfig(
            cache_dir=Path("/tmp/yata-cache"),
            max_cache_size_mb=500,
        )
        assert config.cache_dir == Path("/tmp/yata-cache")
...
class TestMCPConfig:
    ...
    def test_mcp_config_custom_values(self) -> None:
        ...
        config = MCPConfig(
            server_name="custom-yata",
            version="2.0.0",
        )
        assert config.server_name == "custom-yata"
        assert config.version == "2.0.0"


class TestYataConfig:
    """Test main YataConfig."""

    @pytest.mark.unit
    def test_yata_config_defaults(self) -> None:
        """Test YataConfig has all required sections."""
        config = YataConfig()
    ...
    def test_yata_config_from_env(self) -> None:
        """Test YataConfig reads from environment variables."""
        os.environ["YATA_LOG_LEVEL"] = "DEBUG"
        try:
            config = YataConfig()
            assert config.log_level == LogLevel.DEBUG
        finally:
            del os.environ["YATA_LOG_LEVEL"]

    @pytest.mark.unit
    def test_yata_config_nested_env(self) -> None:
        """Test nested config from environment."""
        os.environ["YATA_STORAGE__MAX_CACHE_SIZE_MB"] = "1000"
        try:
            config = YataConfig()
            assert config.storage.max_cache_size_mb == 1000
        finally:
            del os.environ["YATA_STORAGE__MAX_CACHE_SIZE_MB"]
