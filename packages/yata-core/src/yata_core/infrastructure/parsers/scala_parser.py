...
class ScalaParser:
    ...
    def _extract_class(self, node: Node, code: str, file_path: str, entities: list, relationships: list, parent_id: str | None) -> None:
        ...
        if body_node:
            for child in body_node.children:
                self._extract_from_node(child, code, file_path, entities, relationships, [], [], class_id, class_id)
    ...
    def _extract_object(self, node: Node, code: str, file_path: str, entities: list, relationships: list, parent_id: str | None) -> None:
        ...
        if body_node:
            for child in body_node.children:
                self._extract_from_node(child, code, file_path, entities, relationships, [], [], object_id, object_id)
    
    def _extract_trait(self, node: Node, code: str, file_path: str, entities: list, relationships: list, parent_id: str | None) -> None:
        name_node = node.child_by_field_name("name")
        if not name_node:
            return
        ...
        if parent_id:
            relationships.append(Relationship(source_id=EntityId(value=parent_id), target_id=EntityId(value=trait_id), type=RelationshipType.CONTAINS))
