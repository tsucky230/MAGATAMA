...
class PhpParser:
    ...
    def _extract_from_node(self, node: Node, code: str, file_path: str, entities: list, relationships: list, imports: list, errors: list, parent_id: str | None = None, current_class_id: str | None = None) -> None:
        if node.type == "class_declaration":
            ...
            self._extract_namespace(node, code, file_path, entities, relationships, imports, errors, parent_id)
            return
    ...
    def _extract_namespace(self, node: Node, code: str, file_path: str, entities: list, relationships: list, imports: list, errors: list, parent_id: str | None) -> None:
        name_node = node.child_by_field_name("name")
        if not name_node:
            return
        ...
        if parent_id:
            relationships.append(Relationship(source_id=EntityId(value=parent_id), target_id=EntityId(value=namespace_id), type=RelationshipType.CONTAINS))
        
        body_node = node.child_by_field_name("body")
        if body_node:
            for child in body_node.children:
                self._extract_from_node(child, code, file_path, entities, relationships, imports, errors, namespace_id, None)
