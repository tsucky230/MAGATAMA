...
class KotlinParser:
    """Parser for Kotlin source code using tree-sitter."""
    ...
    def _extract_from_node(self, node: Node, code: str, file_path: str, entities: list, relationships: list, imports: list, errors: list, parent_id: str | None = None, current_class_id: str | None = None) -> None:
        if node.type == "class_declaration":
            ...
            self._extract_function(node, code, file_path, entities, relationships, parent_id, current_class_id)
    ...
    def _extract_class(self, node: Node, code: str, file_path: str, entities: list, relationships: list, parent_id: str | None) -> None:
        name_node = None
        for child in node.children:
        ...
        for child in node.children:
            if child.type == "class_body":
                for subchild in child.children:
                    self._extract_from_node(subchild, code, file_path, entities, relationships, [], [], class_id, class_id)
    
    def _extract_object(self, node: Node, code: str, file_path: str, entities: list, relationships: list, parent_id: str | None) -> None:
        name_node = None
        for child in node.children:
            if child.type in ["type_identifier", "identifier"]:
                name_node = child
        ...
        if parent_id:
            relationships.append(Relationship(source_id=EntityId(value=parent_id), target_id=EntityId(value=object_id), type=RelationshipType.CONTAINS))
        
        for child in node.children:
            if child.type == "class_body":
                for subchild in child.children:
                    self._extract_from_node(subchild, code, file_path, entities, relationships, [], [], object_id, object_id)
    
    def _extract_interface(self, node: Node, code: str, file_path: str, entities: list, relationships: list, parent_id: str | None) -> None:
        name_node = None
        for child in node.children:
            if child.type in ["type_identifier", "identifier"]:
                name_node = child
        ...
        if parent_id:
            relationships.append(Relationship(source_id=EntityId(value=parent_id), target_id=EntityId(value=interface_id), type=RelationshipType.CONTAINS))
    
    def _extract_function(self, node: Node, code: str, file_path: str, entities: list, relationships: list, parent_id: str | None, current_class_id: str | None) -> None:
        name_node = None
        for child in node.children:
    ...
    def _extract_package(self, node: Node, code: str, file_path: str, entities: list, relationships: list, parent_id: str | None) -> None:
        for child in node.children:
            if child.type == "identifier":
                ...
                if parent_id:
                    relationships.append(Relationship(source_id=EntityId(value=parent_id), target_id=EntityId(value=package_id), type=RelationshipType.CONTAINS))
                break
