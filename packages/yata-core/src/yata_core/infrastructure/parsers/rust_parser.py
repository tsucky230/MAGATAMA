...
class RustParser:
    ...
    def _extract_from_node(
        ...
        if node.type == "function_item":
            self._extract_function(
                node, file_path, entities, relationships, parent_id
            )
            ...
            self._extract_mod(
                node, file_path, entities, relationships, imports, errors, parent_id
            )
        ...
        for child in body_node.children:
            if child.type == "function_item":
                self._extract_method(
                    child, file_path, entities, relationships, parent_id, class_id
                )
    ...
    def _extract_use(self, node: Node, imports: list[str]) -> None:
        ...
        # Get the use path
        for child in node.children:
            if child.type == "use_clause" or child.type == "scoped_use_list":
                path = child.text.decode("utf-8")
                imports.append(path)
            elif child.type == "identifier" or child.type == "scoped_identifier":
                path = child.text.decode("utf-8")
                imports.append(path)
