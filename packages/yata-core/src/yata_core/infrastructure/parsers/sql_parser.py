...
class SqlParser:
    ...
    def _extract_table(self, node: Node, code: str, file_path: str, entities: list, relationships: list, parent_id: str | None) -> None:
        table_name = None
        for child in node.children:
        ...
        if parent_id:
            relationships.append(Relationship(source_id=EntityId(value=parent_id), target_id=EntityId(value=table_id), type=RelationshipType.CONTAINS))
        
        # Extract columns
        self._extract_columns(node, code, file_path, entities, relationships, table_id)
    
    def _extract_columns(self, node: Node, code: str, file_path: str, entities: list, relationships: list, table_id: str) -> None:
        for child in node.children:
            if child.type in ["column_definition", "column_def"]:
                ...
                for subchild in child.children:
                    if subchild.type in ["column_definition", "column_def"]:
                        self._extract_column(subchild, code, file_path, entities, relationships, table_id)
    
    def _extract_column(self, node: Node, code: str, file_path: str, entities: list, relationships: list, table_id: str) -> None:
        col_name = None
        for child in node.children:
        ...
        entities.append(entity)
        relationships.append(Relationship(source_id=EntityId(value=table_id), target_id=EntityId(value=col_id), type=RelationshipType.CONTAINS))
    
    def _extract_view(self, node: Node, code: str, file_path: str, entities: list, relationships: list, parent_id: str | None) -> None:
        view_name = None
        for child in node.children:
        ...
        if parent_id:
            relationships.append(Relationship(source_id=EntityId(value=parent_id), target_id=EntityId(value=view_id), type=RelationshipType.CONTAINS))
    ...
    def _extract_index(self, node: Node, code: str, file_path: str, entities: list, relationships: list, parent_id: str | None) -> None:
        index_name = None
        for child in node.children:
        ...
        if parent_id:
            relationships.append(Relationship(source_id=EntityId(value=parent_id), target_id=EntityId(value=idx_id), type=RelationshipType.CONTAINS))
    
    def _extract_trigger(self, node: Node, code: str, file_path: str, entities: list, relationships: list, parent_id: str | None) -> None:
        trigger_name = None
        for child in node.children:
        ...
        if parent_id:
            relationships.append(Relationship(source_id=EntityId(value=parent_id), target_id=EntityId(value=trigger_id), type=RelationshipType.CONTAINS))
