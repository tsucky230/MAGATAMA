...
class PythonParser:
    ...
    def _extract_entities(
        ...
        self._visit_node(root, code, file_path, result, parent_class_id=None, function_name_to_id=function_name_to_id)
        ...
        if node.type == "function_definition":
            ...
            if body:
                for child in body.children:
                    self._visit_node(child, code, file_path, result, class_entity.id, function_name_to_id)
        else:
            # Continue traversing
            for child in node.children:
                self._visit_node(child, code, file_path, result, parent_class_id, function_name_to_id)
    ...
    def _find_calls_in_node(
        ...
        if node.type == "function_definition":
            ...
            for child in node.children:
                self._find_calls_in_node(child, code, result, entity_name_to_id, current_function_id)
        elif node.type == "class_definition":
                    ...
                    if import_list:
                        for item in import_list.children:
                            if item.type in ("dotted_name", "identifier"):
                                ...
                                name_node = self._find_child(item, "identifier") or self._find_child(item, "dotted_name")
                                if name_node:
