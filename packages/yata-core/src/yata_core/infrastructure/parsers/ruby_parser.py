...
class RubyParser:
    ...
    def _extract_from_node(
        ...
        if node.type == "class":
            ...
            self._extract_method(
                node, code, file_path, entities, relationships,
                parent_id, current_class_id, is_singleton=False,
            )
            return
            ...
            self._extract_method(
                node, code, file_path, entities, relationships,
                parent_id, current_class_id, is_singleton=True,
            )
            return
        ...
        for child in node.children:
            self._extract_from_node(
                ...
                imports, errors, parent_id, current_class_id,
            )
    ...
    def _extract_class(
        ...
        if body_node:
            for child in body_node.children:
                self._extract_from_node(
                    child, code, file_path, entities, relationships,
                    imports, errors, parent_id=class_id, current_class_id=class_id,
                )
    ...
    def _extract_module(
        ...
        if body_node:
            for child in body_node.children:
                self._extract_from_node(
                    child, code, file_path, entities, relationships,
                    imports, errors, parent_id=module_id, current_class_id=None,
                )
    ...
    def _extract_method(
        ...
        if params_node:
            for param in params_node.children:
                if param.type in ("identifier", "optional_parameter", 
    ...
    def _extract_imports_relationships(
        ...
        for child in node.children:
            self._extract_imports_relationships(
                child, code, relationships, module_id,
            )
